...
expectBetaEq((s1, e1@BoolType()), (_, Infer(e2))) :- e1 == e2.
	\end{lstlisting}
	\item A complex expression \verb|e1| on the left, an infer expression on the right. 
	\item An infer expression on the left, any expression on the right.
	\item An infer expression on the left and on the right.
\end{enumerate}
